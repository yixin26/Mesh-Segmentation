...
public:
	...
	double trace_back(SurfacePoint& destination,		//trace back piecewise-linear path
...
inline long GeodesicAlgorithmExact::visible_from_source(SurfacePoint& point)	//negative if not visible
{
	...
	if(point.type() == EDGE)		
	{
		...
		/////////////////after update
		double position = point.distance(e->v0())/(e->v0()->distance(e->v1()))*e->length();
...
inline void GeodesicAlgorithmExact::initialize_propagation_data()
{
	...
	for(unsigned i=0; i<m_sources.size(); ++i)		//for all edges adjacent to the starting vertex			
	{
		...
		if(source->base_element()->type()==EDGE){
			edge_pointer e = static_cast<edge_pointer>(source->base_element());
			double mstop= source->distance(e->v0())/(e->v0()->distance(e->v1()))*e->length();

			if(mstop!=0){
				candidate.initialize(e, source, i);
				candidate.start() = 0;
				candidate.stop() = mstop;
				candidate.pseudo_x()= mstop;
			...
			if(mstop!=e->length()){
				IntervalWithStop candidate2;
				candidate2.initialize(e, source, i);
				candidate2.start() = mstop;
				candidate2.stop() = e->length();
				candidate2.compute_min_distance(candidate2.stop());
				candidate2.direction() = Interval::FROM_SOURCE;

				update_list_and_queue(interval_list(e), &candidate2, 1);
...
inline double GeodesicAlgorithmExact::trace_back(SurfacePoint& destination,		//trace back piecewise-linear path
										std::vector<SurfacePoint>& path)
{					
	path.clear();
	double best_total_distance; 
	...
	if(best_interval)	//if we did not hit the face source immediately  
	{
		...
		while(visible_from_source(path.back()) < 0 && path.size()<m_edge_interval_lists.size())		//while this point is not in the direct visibility of some source (if we are inside the FACE, we obviously hit the source)
		{
	...
	if(path.back().distance(&source) > 1e-9)
	{
