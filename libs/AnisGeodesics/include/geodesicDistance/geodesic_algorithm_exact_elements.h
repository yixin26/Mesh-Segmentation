...
public:
	...
	void local_coordinates(SurfacePoint* point,
		double& x, 
		double& y)
	{
		double d0,d1;
		if(point->type() == VERTEX)
		{
			...
			for(unsigned i=0; i<v->adjacent_faces().size(); ++i)
			{
				...
				if(f->opposite_edge(v)->id()==m_edge->id()){
					d0=f->next_edge(m_edge,m_edge->v0())->length();
					d1=f->next_edge(m_edge,m_edge->v1())->length();
					break;
				}
			}
		}
		else if(point->type() == EDGE)
		{
			...
			double c0=point->distance(e->v0())/(e->v0()->distance(e->v1()))*e->length();
			double c1=e->length()-c0;
			double el=e->length();
			double ang;
			for(unsigned i=0; i<e->adjacent_faces().size(); ++i)
			{
				face_pointer f = e->adjacent_faces()[i];
				if(f->next_edge(e,e->v0())->id()==m_edge->id()){
					if(e->v0()==m_edge->v0()){
						d0=c0;
						for(unsigned j=0;j<3;j++){
							if(f->adjacent_vertices()[j]==e->v0()){
								ang= f->corner_angles()[j]; break;}
						}
						d1= sqrt(d0*d0+el*el-2*d0*el*cos(ang));
					}
					else{
						d1=c0;
						for(unsigned j=0;j<3;j++){
						...
						d0= sqrt(d1*d1+el*el-2*d1*el*cos(ang));
					}
					break;
				}
				else if(f->next_edge(e,e->v1())->id()==m_edge->id()){
					if(e->v1()==m_edge->v0()){
						d0=c1;
						for(unsigned j=0;j<3;j++){
							if(f->adjacent_vertices()[j]==e->v1()){
								ang= f->corner_angles()[j];break;}
						}
						d1= sqrt(d0*d0+el*el-2*d0*el*cos(ang));
					}
		...
		x = m_edge->length()/2.0 + (d0*d0 - d1*d1)/(2.0*m_edge->length());
			...
			for(unsigned i=0; i<e->adjacent_faces().size(); ++i)
			{
				...
				if(f->next_edge(e,e->v0())->id()==m_edge->id()){
					if(e->v0()==m_edge->v0()){
						d0=c0;
						for(unsigned j=0;j<3;j++){
							if(f->adjacent_vertices()[j]->id()==e->v0()->id()){
								ang= f->corner_angles()[j]; break;}
						}
						d1= sqrt(d0*d0+el*el-2*d0*el*cos(ang));
					}
					...
					if(e->v1()==m_edge->v0()){
						d0=c1;
						for(unsigned j=0;j<3;j++){
							if(f->adjacent_vertices()[j]->id()==e->v1()->id()){
								ang= f->corner_angles()[j];break;}
						}
						d1= sqrt(d0*d0+el*el-2*d0*el*cos(ang));
					}
								 ...
								 unsigned source_index)
{
	...
	local_coordinates(source, m_pseudo_x, m_pseudo_y);
